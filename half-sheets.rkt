...
(define GALLIUM_ICON
 (scale (bitmap "galliumos.jpg") 0.2))
...
(define (path-item a b)
  (let* ([is-file (string-contains? a ".")]
         [folder-bullet (scale-to-fit (bitmap "folder-icon.png") 20 20)]
         [file-bullet (scale-to-fit (bitmap "file-icon.png") 20 20)]
...
(define (display-video-qr a)
  (let* ([maybe-tag a]
         [icon (if (tagged-link? maybe-tag)
                   (scale-to-fit (bitmap (++ (tagged-link-tag maybe-tag) "-icon.png")) 20 20)
                   (blank 0))]
...
(define (activity-repeat->pict a)
  (let* [(main
        ...
        (label-with-arrows (hc-append (cc-superimpose
                                       (scale-to-fit (bitmap "repeat-icon.png") 100 100)
                                       (t times))
                                      label))]
    (vc-append 20
               (activity-header a)
...
(define (award-amount->pict amount)
  (let ([icon (scale (bitmap "coin.png") 0.5)])
   (vc-append (colorize
